...
io.on('connect', async socket => {
  ...
  socket.on('addFriend', async (token, friendID, callback) => {
    try {
      ...
      let conversation = await Conversation.findOne({ users: [user._id, friendID], isAutogenerated: true });
      ...
      if (!conversation) {
        conversation = new Conversation({ users: [user._id, friendID] });
  ...
  socket.on('removeFriend', async (token, friendID, callback) => {
    try {
      ...
      await Conversation.deleteOne({ users: { $in: [user._id, friendID] }, isAutogenerated: true });
  ...
  socket.on('getFriends', async (token, callback) => {
    try {
      const { friends } = await authorize(token);
  ...
  socket.on('getUserConversations', async (token, callback) => {
    try {
      const user = await authorize(token);
      const conversations = await Conversation.find({ users: { $in: [user._id] } }).populate('users');
  ...
  socket.on('getConversationMessages', async (token, conversationID, callback) => {
    try {
      ...
      if (conversation) callback(conversation.messages.slice(-16));
    } catch (error) {
      console.error(error);
    }
  });
